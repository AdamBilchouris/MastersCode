...
# load
load("1d_gauss_0_40_2001points_var_0.988.Rdata")
...
xNew[501]
plot(xNew[1:501], vals, type='o', ylim=c(-0.1, 1))
lines(xNew[1:501], exp(-x[1:501]^2), lty=2, lwd=2)
...
xNew[501]
plot(xNew[1:501], vals2, type='o', ylim=c(-0.1, 1))
lines(xNew[1:501], exp(-x[1:501]^2), lty=2, lwd=2)
...
xNew[501]
plot(xNew[1:501], vals3, type='o', ylim=c(-0.1, 1))
lines(xNew[1:501], exp(-x[1:501]^2), lty=2, lwd=2)
...
plot(tVals, vals_hall, type='l', ylim=c(-1, 1.1))
lines(tVals, vals_hall_orig, lty=3, lwd=2, col=3)
lines(tVals, exp(-tVals^2), lty=2, lwd=2, col=2)
...
lines(tVals, exp(-tVals^2), col=2, lty=2, lwd=2)

# Genton Qn covariance
rob_est <- robacf(z, type='covariance', lag.max=501, plot=F)
rob_est_vals <- rob_est$acf[, , 1]
rob_est_cor_vals <- robacf(z, type='correlation', lag.max=501, plot=F)$acf[,,1]
...
valsDf <- data.frame(x=xNew[1:501])
for(j in 1:(m+p)) {
  jStr <- glue('j{j}') 
...
wlsDf <- data.frame(lags=x[1:501], 'j1'=valsDf$j1, 'j2'=valsDf$j2, 'j3'=valsDf$j3, 'j4'=valsDf$j4, 'j5'=valsDf$j5, estCov=vals)
...
# Plot!
# Autocorrelation
plotName <- glue("waves_autocorr_500.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:501], exp(-xNew[1:501]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(rho)(h)), ylim=c(-0.35, 1.1), axes=T)
ticks <- c(-0.35, -0.15, 0.25, 0.75)
axis(side=2, at=ticks)
lines(xNew[1:501], vals / vals[1], col=2, lty=2, lwd=2)
lines(xNew[1:501], vals2 / vals2[1], col=3, lty=3, lwd=2)
lines(xNew[1:501], vals3 / vals3[1], col=4, lty=4, lwd=2)
lines(xNew[1:501], vals_hall_2_idct / vals_hall_2_idct[1], col=6, lty=5, lwd=2)
lines(xNew[1:501], rob_est_cor_vals, col=7, lty=6, lwd=2)
lines(xNew[1:501], tapered / tapered[1], col=8, lty=8, lwd=2)
lines(xNew[1:501], splineVals / splineVals[1], col=13, lty=9, lwd=2)
legend('topright', c('True', expression('C'^'*'*'(h)'), expression('C'^'**'*'(h)'), expression('C'^'(a)'*'(h)'),
...
# Autocovariance
plotName <- glue("waves_autocov_500.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:501], exp(-xNew[1:501]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(C)(h)), ylim=c(-0.45, 1.1))
axis(side=2, at=ticks)
lines(xNew[1:501], vals, col=2, lty=2, lwd=2)
lines(xNew[1:501], vals2, col=3, lty=3, lwd=2)
lines(xNew[1:501], vals3, col=4, lty=4, lwd=2)
lines(xNew[1:501], vals_hall_2_idct, col=6, lty=5, lwd=2)
lines(xNew[1:501], rob_est_vals, col=7, lty=6, lwd=2)
lines(xNew[1:501], tapered, col=8, lty=8, lwd=2)
lines(xNew[1:501], splineVals, col=13, lty=9, lwd=2)
legend('topright', c('True', expression('C'^'*'*'(h)'), expression('C'^'**'*'(h)'), expression('C'^'(a)'*'(h)'),
                     expression(tilde('C')*'(h)'), expression(hat('C')[Q]*'(h)'), expression(hat('C')[N]^'a'*'(h)'), expression(hat('C')^'B'*('h'))),
       lty=c(1, 2, 3, 4, 5, 6, 8, 9), col=c(1, 2, 3, 4, 6, 7, 8, 13), lwd=c(2, 2, 2, 2, 2, 2, 2, 2), y.intersp=1.2, cex=1.2)
dev.off()
