...
model <- RMbessel(nu=-0.49, var=1, scale=1)
...
# Estimator functions
{
  ...
  yaglom_1.76 <- function(X, limT, tau, trueTau, k_t, meanX) {
    ...
    vals <- sapply(seq(1, limT - tau, by=1), function(t) (X[t+tau] - meanX)*(X[t] - meanX))
    return((sum(vals) / limT) * kern_wave_yaglom(tau, k_t))
  }
}
...
# Error metrics
{
  ...
  compute_dists <- function(x, upperX, randomPoint, spatDf, spatGrid, zeroIdx) {
  ...
  bessel_kriging <- function(estY, x, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx) {
    ...
    for(i in 1:length(randomPoints)) {
      covList <- generate_cov_matrix_vector(x, upperX, randomPoints[i], spatDf[-(r - zeroIdx + 1), ], spatGrid[r, ], estY, 
...
outsideXIdx_0_40_idx <- c(outsideXIdx, zeroIdx:fortyIdx)
outsideXIdx_0_40_idx  <- sort(outsideXIdx_0_40_idx)
...
fe <- foreach(it=1:iters, .packages=c("RandomFields", "gstat", "gsignal", "tsqn", "pracma", "minpack.lm", "glue", "dfoptim"), export=ls(globalenv())) %dopar% {
  ...
  resListInner[["Cs"]][["errDf"]][1, 'mspe'] <- bessel_kriging(vals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["Cs"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(vals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["Css"]][["errDf"]][1, 'mspe'] <- bessel_kriging(vals2, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["Css"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(vals2, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  zMean <- mean(z)
  for(i in 0:(upperX - 1)) {
    vals3 <- c(vals3, yaglom_1.76(z, length(z), i, xNew[i+1], length(z), zMean))
  }
  ...
  resListInner[["C(a)"]][["errDf"]][1, 'mspe'] <- bessel_kriging(vals3, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["C(a)"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(vals3, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["hall"]][["errDf"]][1, 'mspe'] <- bessel_kriging(vals_hall_2_idct, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["hall"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(vals_hall_2_idct, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CQ"]][["errDf"]][1, 'mspe'] <- bessel_kriging(rob_est_vals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CQ"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(rob_est_vals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CNa"]][["errDf"]][1, 'mspe'] <- bessel_kriging(tapered, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CNa"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(tapered, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CB"]][["errDf"]][1, 'mspe'] <- bessel_kriging(splineVals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CB"]][["errDf"]][1, 'mspe_gstat'] <- bessel_kriging_gstat(splineVals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
...
for(n in estNames) {
  ...
  # PLOT
  plot(xNew[1:upperX], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:upperX], nu=model@par.model$nu)) / (xNew[1:upperX]^(model@par.model$nu)))[-1]), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(rho(h)), ylim=c(-1, 1))
...
plotName <- glue("bessel_corr/bands.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:upperX], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:upperX], nu=model@par.model$nu)) / (xNew[1:upperX]^(model@par.model$nu)))[-1]), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(rho)*'(h) bands'), ylim=c(-1, 1))
...
knitr::kable(resDf2, format='latex', align='c', booktabs=F, escape=F, digits=64)
