...
load("from_uni_pc/Bessel_n100_p100_res_0.1_horizontal_vertical.RData")

seqRF <- seq(0, 25.8, by=0.1)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_eps_0.1_cardinal_y.RData")
seqRF <- seq(0, 25.8, by=0.1)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_eps_0.2_cardinal_y.RData")
seqRF <- seq(0, 25.8, by=0.1)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_eps_0.25_cardinal_y.RData")
seqRF <- seq(0, 25.8, by=0.1)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_eps_0.3_cardinal_y.RData")
seqRF <- seq(0, 25.8, by=0.1)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_andriy_epsilon_2_kep_20.RData")
seqRF <- seq(0.1, 25.8, by=0.1)

trueY <- c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(seqRF, nu=model@par.model$nu)) / (seqRF^(model@par.model$nu)))[-1])
estY <- acf_fin
estY <- estY[1:length(seqRF)]

plot(seqRF, trueY, ylim=c(-0.3, 1.1))
lines(seqRF, estY, col='red', lty=2, lwd=2)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_fourier.RData")
seqRF <- fsList[[1]]$x

# find closest to 25.8
fourierIndex <- which(round(seqRF, 1) == 25.8)
...
load("from_uni_pc/Bessel_n100_p100_res_0.1_RFcov(data=sim, bin=seq(0, 280, by=0.1)).RData")
seqRF <- seq(0, 25.8, by=0.1)
seqRF <- estRFcov@centers
empCovClosest <- which(plyr::round_any(seqRF, 0.1) == 25.8)[1]
