...
load("1d_bessel_nu_n0.49_n10_50_3001points_origVar_1.004_newVar_1.006.RData")

model <- RMbessel(var=1, scale=1, nu=-0.49)
...
xNew[256]
plot(xNew[1:256], vals, type='o', ylim=c(-1, 1))
lines(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]),
      col=2, lty=2, lwd=2, type='l', xlab=expression(tau), ylab=expression(rho(tau)), ylim=c(-1, 1))
...
xNew[256]
plot(xNew[1:256], vals2, type='o', ylim=c(-1, 1))
lines(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]),
      col=2, lty=2, lwd=2, type='l', xlab=expression(tau), ylab=expression(rho(tau)), ylim=c(-1, 1))
...
xNew[256]
plot(xNew[1:256], vals3, type='o', ylim=c(-1, 1))
lines(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]),
      col=2, lty=2, lwd=2, type='l', xlab=expression(tau), ylab=expression(rho(tau)), ylim=c(-1, 1))
...
kern_wave_single <- function(tau, theta) {
  return( ifelse(tau == 0, (1 / (pi*theta)), ((theta/tau) * sin(tau/theta)) / (pi*theta)) )
}

kern_wave_hall <- function(tau, theta) {
  return( sapply(tau, kern_wave_single, theta=theta) )
}
...
plot(tVals, vals_hall, type='l', ylim=c(-3, 3))
lines(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]),
      col=2, lty=2, lwd=2, type='l', xlab=expression(tau), ylab=expression(rho(tau)), ylim=c(-1, 1))
...
plot(tVals, vals_hall_2_idct, type='l')
...
# Plot!
# Autocorrelation
plotName <- glue("bessel_autocorr.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(rho)(h)), ylim=c(-1, 1))
...
# Autocovariance
plotName <- glue("bessel_autocov.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:256], c(1, ((2^(model@par.model$nu) * gamma(model@par.model$nu+1) * besselJ(xNew[1:256], nu=model@par.model$nu)) / (xNew[1:256]^(model@par.model$nu)))[-1]), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(C)(h)), ylim=c(-1, 1.3))
