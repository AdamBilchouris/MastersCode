...
# Estimator functions
{
  ...
  yaglom_1.76 <- function(X, limT, tau, trueTau, k_t, meanX) {
    ...
    vals <- sapply(seq(1, limT - tau, by=1), function(t) (X[t+tau] - meanX)*(X[t] - meanX))
    return((sum(vals) / limT) * kern_gaussian_yaglom(tau, k_t))
  }
}
  ...
  acf_at_t2_trunc <- function(X, meanX, t, x, T1, T2, h, X_ij_mat) {
    ...
    if(t >= 0 && t <= T1) {
      for(i in 1:length(x)) {
        ...
        t_tij <- t - tij
        X_ij <- X_ij_mat[i, ]
        K_ij <- kern_gaussian_hall(t_tij, h)
        numerator <- K_ij * X_ij
        denominator <- K_ij
...
# Error metrics
{
  ...
  # x = seqRF
  gauss_kriging <- function(estY, x, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx) {
  ...
  gauss_kriging_gstat <- function(estY, lags, x, upperX, randomPoints, spatDf, spatGrid, zOrig) {
    nlsDf <- data.frame(x=lags, vals=estY)
    nlsControl <- nls.control(maxiter=400)
    fitModel <- nls(vals ~ exp(-x^2 / a^2), data=nlsDf, start=list(a=1), weights=rep(1/nrow(nlsDf), nrow(nlsDf)), control=nlsControl)
    m <- vgm(1, 'Gau', coef(fitModel)[[1]], nugget=0.0000001)
    zz <- c()
...
fe <- foreach(it=1:iters, .packages=c("RandomFields", "gstat", "gsignal", "tsqn", "pracma", "minpack.lm", "glue", "dfoptim"), export=ls(globalenv())) %dopar% {
  ...
  resListInner[["Cs"]][["errDf"]][1, 'mspe'] <- gauss_kriging(vals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["Cs"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(vals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["Css"]][["errDf"]][1, 'mspe'] <- gauss_kriging(vals2, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["Css"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(vals2, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  zMean <- mean(z)
  for(i in 0:(upperX - 1)) {
    vals3 <- c(vals3, yaglom_1.76(z, length(z), i, xNew[i+1], 0.1*length(z), zMean))
  }
  ...
  resListInner[["C(a)"]][["errDf"]][1, 'mspe'] <- gauss_kriging(vals3, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["C(a)"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(vals3, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["hall"]][["errDf"]][1, 'mspe'] <- gauss_kriging(vals_hall_2_idct, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["hall"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(vals_hall_2_idct, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CQ"]][["errDf"]][1, 'mspe'] <- gauss_kriging(rob_est_vals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CQ"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(rob_est_vals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CNa"]][["errDf"]][1, 'mspe'] <- gauss_kriging(tapered, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CNa"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(tapered, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["CB"]][["errDf"]][1, 'mspe'] <- gauss_kriging(splineVals, seqRF, upperX, randomPoints, dists_withinList, spatDf, spatGrid, z, zOrig, zeroIdx)
  resListInner[["CB"]][["errDf"]][1, 'mspe_gstat'] <- gauss_kriging_gstat(splineVals, lags=xNew[1:upperX], xNew, upperX, randomPoints, spatDf, spatGrid, zOrig)
  ...
  resListInner[["mspeRank"]] <- rank(c(resListInner[["Cs"]][["errDf"]][1, 'mspe'], resListInner[["Css"]][["errDf"]][1, 'mspe'],
                                       ...
                                       resListInner[["CQ"]][["errDf"]][1, 'mspe'], resListInner[["CNa"]][["errDf"]][1, 'mspe'], resListInner[["CB"]][["errDf"]][1, 'mspe_gstat']))
...
for(n in estNames) {
  ...
  # PLOT
  plot(seqRF[1:upperX], exp(-seqRF[1:upperX]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(rho(h)), ylim=c(-1, 1))
  lines(seqRF[1:upperX], minACF_C_star, lwd=2, lty=1, col=colours[count])
  lines(seqRF[1:upperX], maxACF_C_star, lwd=2, lty=1, col=colours[count])
  
  polygon(c(seqRF[1:upperX], rev(seqRF[1:upperX])), c(minACF_C_star, rev(maxACF_C_star)), col=adjustedColours[count])
  count <- count + 1
}
...
plotName <- glue("gauss_corr/bands.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(seqRF[1:upperX], exp(-seqRF[1:upperX]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(rho)*'(h) bands'), ylim=c(-1, 1))
...
for(n in estNames) {
  ...
  polygon(c(seqRF[1:upperX], rev(seqRF[1:upperX])), c(minACF_C_star, rev(maxACF_C_star)), col=adjustcolor(colours[count], alpha=0.2), lty=0)
  count <- count + 1
}
adjustedColours <- sapply(colours, function(c) adjustcolor(c, alpha=0.2))
...
for(n in estNames) {
  resDf[n, ] <- colMeans(resList[[n]][["errDf"]][-2,])
}
