...
load("1d_gauss_n10_50_3001points_origVar_1.015_newVar_1.001.Rdata")
...
lines(xNew[1:256], exp(-xNew[1:256]^2), lty=2, lwd=2)
...
# Plot!
# Autocorrelation
plotName <- glue("gauss_autocorr.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:256], exp(-xNew[1:256]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(rho)(h)), ylim=c(-0.35, 1.1), axes=T)
...
# Autocovariance
plotName <- glue("gauss_autocov.pdf")
pdf(plotName, width=10, height=9.6)
par(mar=c(4,4.5,0.25,0.25)+.1)
plot(xNew[1:256], exp(-xNew[1:256]^2), col=1, lty=1, lwd=2, type='l', xlab=expression(h), ylab=expression(hat(C)(h)), ylim=c(-0.35, 1.1))
ticks <- c(-0.4, -0.25, 0.25, 0.75, 1.25)
axis(side=2, at=ticks)
